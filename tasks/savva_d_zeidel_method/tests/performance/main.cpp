...
class SavvaDZeidelPerfTest : public ppc::util::BaseRunPerfTests<InType, OutType> {
 protected:
  ...
  void SetUp() override {
    ...
    input_data_.n = n;
    input_data_.a.assign(n * n, 0.0);
    input_data_.b.assign(n, 0.0);
    right_output_data_.resize(n);

    for (int i = 0; i < n; ++i) {
      right_output_data_[i] = std::sin(0.001 * i) + 2.0;
    }
    ...
    for (int i = 0; i < n; ++i) {
      ...
      input_data_.a[i * n + i] = diag_sum + 10.0;
    }

    for (int i = 0; i < n; ++i) {
      double sum = 0.0;
      for (int j = 0; j < n; ++j) {
        sum += input_data_.a[i * n + j] * right_output_data_[j];
      }
      input_data_.b[i] = sum;
    }
  }
  ...
  bool CheckTestOutputData(OutType &output_data) final {
    ...
    for (size_t i = 0; i < output_data.size(); ++i) {
      if (std::abs(output_data[i] - right_output_data_[i]) > 0.0001) {
        return false;
      }
    }
    return true;
